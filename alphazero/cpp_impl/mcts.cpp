...
MCTS::MCTS(Env env, A2CLearner a2c_agent, Params params) : params(params), a2c_agent(a2c_agent) {
  auto obs = env.reset();
...
MCTS::_ucb(std::shared_ptr<Node> parent_node, std::shared_ptr<Node> child_node, torch::Tensor action_probs) {
  ...
  double base = *std::get_if<double>(&params["pb_c_base"]);
  double pb_c_init = *std::get_if<double>(&params["pb_c_init"]);
...
MCTS::_gen_children_nodes(std::shared_ptr<Node> parent_node) {
  int n_actions = *std::get_if<int>(&params["n_actions"]);
...
MCTS::_get_best_node(std::shared_ptr<Node> parent_node) {
  ...
  if (cached_action_probs == policy_net_cache.end()) {
    std::tie(action_probs, std::ignore) = a2c_agent.predict_policy({parent_node->state});
...
std::vector<double>
MCTS::policy(Env env, std::vector<double> obs, bool ret_node) {
  // TODO Move to constructor.
  int n_actions = *std::get_if<int>(&params["n_actions"]);
  int n_iter = *std::get_if<int>(&params["simulations"]);
  ...
  torch::Tensor action_probs;
  std::tie(action_probs, std::ignore) = a2c_agent.predict_policy({root_node->state});
  double alpha = *std::get_if<double>(&params["dirichlet_alpha"]);
  double frac = *std::get_if<double>(&params["dirichlet_frac"]);

  std::random_device rd;
  std::mt19937 generator(rd());
  std::gamma_distribution<double> distribution(alpha, 1.);
  for (int i; i < n_actions; ++i) {
  ...
  for (int i = 0; i < n_iter; ++i) {
    ...
    torch::Tensor value;
    std::tie(std::ignore, value) = a2c_agent.predict_policy({node->state});
  ...
  for (auto child : root_node->children) {
    ret.push_back((double) child->visits / (double) n_iter);
  }
  return ret;
}
